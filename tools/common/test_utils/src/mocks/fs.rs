...
impl FileSystem for InMemoryFileSystem {
    ...
    fn remove_file(&self, path: &Path) -> Result<()> {
        let mut fs = self.clone();
        if !fs.files.contains_key(&path.to_path_buf()) {
            return Err(common_errors::WritingError::not_found(&format!(
                "File not found: {}", path.display()
            )));
        }
    ...
    fn remove_dir_all(&self, path: &Path) -> Result<()> {
        let mut fs = self.clone();
        if !fs.dirs.contains(&path.to_path_buf()) {
            return Err(common_errors::WritingError::not_found(&format!(
                "Directory not found: {}", path.display()
            )));
        }
        ...
        fs.files.retain(|file_path, _| {
            !file_path.to_string_lossy().starts_with(&path_str)
        });

        // Remove all directories in this directory and subdirectories
        fs.dirs.retain(|dir_path| {
