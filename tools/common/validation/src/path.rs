...
mod tests {
    ...
    fn test_find_content_path() {
        ...
        let found_path = super::mock_functions::mock_find_content_path("test-post", Some("creativity")).unwrap();
        assert_eq!(found_path.to_string_lossy().to_string(), "/mock/content/creativity/test-post/index.md");
    }
    ...
    fn test_content_exists() {
        ...
        let exists = super::mock_functions::mock_content_exists("test-post", Some("creativity")).unwrap();
        assert!(exists);

        // Test with non-existent content
        let exists = super::mock_functions::mock_content_exists("non-existent", Some("creativity")).unwrap();
        assert!(!exists);
    }
}