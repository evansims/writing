...
fn test_error_context() {
    // Test that error context is properly added
    ...
    assert!(error_msg.contains("While loading configuration"), "Error should contain the context");
    
    // Adding context changes error to Unexpected category - this is the actual behavior
    assert_eq!(ErrorCategory::from(&error), ErrorCategory::Unexpected);
}
...
fn test_nested_error_context() {
    ...
    assert!(error_msg.contains("While validating input"), "Error should contain the first context");
    assert!(error_msg.contains("While processing content"), "Error should contain the second context");
    assert!(error_msg.contains("While building site"), "Error should contain the third context");
