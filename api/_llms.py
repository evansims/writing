...
from api._validation import get_content_path
...
async def llms_txt(request):
    ...
    for content_file in sorted(content_files, key=os.path.getmtime, reverse=True)[:max_entries]:
        try:
